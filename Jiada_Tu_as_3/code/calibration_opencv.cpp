...
{
    float a_e=a.first.first*a.first.first/a.first.second;
    float b_e=b.first.first*b.first.first/b.first.second;
    if(a_e>b_e)
        return a;
    return b;
}

Mat ransacFindH(int n, int d, float p, float w, float kThreshold, vector<Point3f> &world, vector<Point2f> &image, float max_tolerance, float min_tolerance)
{
    float k=log(1-p)/log(1-pow(w, n));
    float t;
    ...
    for(int i=0;i<kThreshold && i<k;i++) {
        ...
        //haven't recompute model based on all inlines. if do recompute, I think the gaussian error can be dealed with.
        int inlines=0;
        float inlinesDist=0;
    ...
    cout<<w<<", "<<k<<", "<<better_H.first.first<<", "<<better_H.first.second<<endl;
...
//    string worldFile("/Users/t/Desktop/test/cc-worldPt.txt");
//    vector<string> imageFiles;
//    imageFiles.push_back("/Users/t/Desktop/test/cc-imagePt.txt");
//    imageFiles.push_back("/Users/t/Desktop/test/cc-imagePt-v1.txt");
//    imageFiles.push_back("/Users/t/Desktop/test/cc-imagePt-v2.txt");
    ...
    vector< vector<Point3f> > worlds;
    for(int i=0;i<images.size();i++)
        worlds.push_back(vector<Point3f>(world));
    
    Mat Kstar;
    Mat distCoeffs;
    vector<Mat> rvecs;
    vector<Mat> tvecs;
    calibrateCamera(worlds, images, Size(640,480), Kstar, distCoeffs, rvecs, tvecs);
    ...
    for (int i=0; i<rvecs.size(); i++) {
        Rodrigues(rvecs[i],rvecs[i]);
        Mat RstarTstar=Mat::zeros(3, 4, CV_32FC1);
        for(int a=0;a<rvecs[i].rows;a++) for(int b=0;b<rvecs[i].cols;b++)
            RstarTstar.at<float>(a,b)=rvecs[i].at<double>(a,b);
        RstarTstar.at<float>(0,3)=tvecs[i].at<double>(0,0);
        RstarTstar.at<float>(1,3)=tvecs[i].at<double>(1,0);
        RstarTstar.at<float>(2,3)=tvecs[i].at<double>(2,0);
        RstarTstars.push_back(RstarTstar);
